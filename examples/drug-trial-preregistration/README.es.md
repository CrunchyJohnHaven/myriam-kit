...
Un ejemplo llave en mano de myriam-kit para patrocinadores de ensayos clínicos e investigadores académicos que desean pre-registrar un ensayo de manera evidente ante manipulación y anclada en Bitcoin **además del** registro en ClinicalTrials.gov, EU CTR u otros registros jurisdiccionales.
...
- ClinicalTrials.gov permite a los patrocinadores actualizar los criterios de valoración, los cálculos de tamaño muestral y los planes de análisis **después** del registro original, con las fechas de modificación visibles en el registro, pero los cambios de *contenido* no siempre quedan anclados criptográficamente.
- Varios estudios de gran escala (Goldacre et al., 2019; Mathieu et al., 2009) han documentado el cambio sistemático de criterios de valoración post-hoc en el campo.
- La aplicación editorial por parte de las revistas (CONSORT, ICMJE) detecta algunos casos; muchos se cuelan.

Un sello MYRIAM cierra esta brecha. La raíz de Merkle + el recibo OTS de Bitcoin hace que cualquier cambio post-hoc al criterio de valoración primario *pre-registrado*, al plan de análisis estadístico o a la lista de subgrupos sea detectable criptográficamente. El sello es, desde el punto de vista forense, más difícil de retconnear que el propio registro.
...
- **Paxil/Study 329 (GSK/SmithKline, 2001)**: datos negativos de un ensayo sobre la conducta suicida en adolescentes se replantearon mediante un análisis post-hoc. Un pre-registro sellado de los criterios de valoración habría hecho que el replanteamiento fuera detectable públicamente.
- **Múltiples ensayos oncológicos** en los que los criterios de valoración de supervivencia fueron sustituidos por criterios de valoración subrogados (supervivencia libre de progresión, tasa de respuesta) cuando el beneficio de supervivencia no llegó a materializarse.
...
- **Revisión del tamaño muestral**: aumentar el tamaño muestral después de ver las tendencias intermedias. El HECHO 06 contiene el tamaño muestral y el cálculo de potencia pre-registrados.
...
- Mala conducta en la ejecución misma del ensayo (datos falsificados, inclusión fraudulenta de pacientes). El sello ancla el *plan*, no la *ejecución*.
...
- Daño a pacientes durante el propio ensayo. El sello es regulatorio y probatorio; no reduce el riesgo asociado a la conducción del ensayo.
...
- Puede asumir la consecuencia: si el ensayo falla en sus criterios de valoración pre-registrados, no puede retconnear silenciosamente hacia un criterio de valoración distinto que sí haya funcionado.
...
- Ensayos de reposicionamiento de medicamentos en biotecnológicas pequeñas sin un largo historial de conducción de ensayos.
...
**TEMPORALIDAD CRÍTICA**: el sello debe crearse ANTES de que comience la inclusión de pacientes. Un pre-registro sellado después de la inclusión del primer paciente es, forensicamente, mucho más débil. Lo ideal es sellar en el momento de la aprobación del IRB.
...
4. **Detección de desviaciones en el plan de análisis.** Un simple diff entre el HECHO 06 sellado y los métodos estadísticos publicados revela cualquier modificación.
...
- No es una vía para anclar el consentimiento del paciente u otros documentos éticos (esos requieren su propio mecanismo de sellado).
